...
type LocationInfoList = Array<LocationInfo>;
type LocationGroup = { [key: string]: LocationInfoList };
type LocationDatabase = { [key: string]: LocationGroup };

/**
 * Create a database with the default set of locations
 */
function database(): LocationDatabase {
...
function _add_location_to_db(location: LocationInfo, db: LocationDatabase) {
    let key = _sanitize_key(location.timezone_group);
    let group = _get_group(key, db);
    if (group == null) {
        group = {};
        db[key] = group;
    }

    let location_key = _sanitize_key(location.name);
    if (location_key in group) {
        group[location_key].push(location);
    } else {
        group[location_key] = [location];
    }
}

function _add_locations_from_str(
...
function _add_locations_from_list(
